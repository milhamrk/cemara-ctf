...
class Profile extends React.Component {
    ...
    componentDidMount() {
        this.fetchScoreboardVisibility();
    ...
    fetchScoreboardVisibility = async () => {
        try {
            const response = await fetch('https://cemara.one/act.json', { mode: 'cors' });
                ...
                if (data.hidden !== true) {
                    ...
                    for (let x = 0; x < challengeArray.length; x++) {
                        ...
                        if (challengeArray[x].points !== 0) {
                            ...
                            const dateTime = Math.abs(new Date() - new Date(currentStuff.timestamp)) / 1000;
    ...
    render() {
        return (
            <Layout className="layout-style">
                ...
                {!this.state.showScoreboard && !this.state.loading && (
                    <div style={{ height: "100%", width: "100%" }}>
                        <br /><br /><br />
                        <Empty
                            image={<FrownOutlined />}
                            imageStyle={{ fontSize: "500%", color: "#177ddc" }}
                            description={<h1>The scoreboard is currently disabled.</h1>}
                        />
                    </div>
                )}
                {this.state.showScoreboard && this.state.targetUser && !this.state.loading && (

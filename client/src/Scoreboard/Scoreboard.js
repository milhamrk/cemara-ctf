...
import { Layout, message, Table, Avatar, Select, Switch } from 'antd';
import {
  FileUnknownTwoTone,
  TeamOutlined,
  ApartmentOutlined
...
class Scoreboard extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      liveUpdates: false,
      categoryListOptions: [],
      isEnabled: true, // Added state variable for enable/disable functionality
      showScoreboard: false // Added state variable for showing/hiding scoreboard
    };
  }

  componentDidMount = async () => {
    try {
      const response = await fetch('https://cemara.one/act.json');
      const data = await response.json();
      this.setState({ showScoreboard: data.scoreboard === 1 });
    } catch (error) {
    ...
    if (!updating) {
      ...
      for (let x = 0; x < scoreArray.length; x++) {
        if ("timestamp" in scoreArray[x] && scoreArray[x].timestamp !== "0") {
          ...
          const dateTime = Math.abs(new Date() - new Date(scoreArray[x].timestamp)) / 1000;
  ...
  connectWebSocket() {
    if (this.state.isEnabled) {
      ...
      webSocket.onmessage = (e) => {
        ...
        if (data.type === "score") {
          updating = true;
          const payloadArray = data.data;
          for (let y = 0; y < payloadArray.length; y++) {
            let userFound = false;
            const payload = payloadArray[y];
            ...
            for (let x = 0; x < changes.users.length; x++) {
              if (changes.users[x]._id === payload.author) {
                ...
                for (let i = 0; i < currentUserChanges.length; i++) {
                  if (currentUserChanges[i]._id === payload._id) {
                    transactionFound = true;
                    currentUserChanges[i] = payload;
                    break userLoop;
                  }
                }
  ...
  render() {
    return (
      <Layout className="layout-style">
        <Switch checked={this.state.showScoreboard} onChange={this.toggleScoreboard} />

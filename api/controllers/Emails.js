...
const adminUnVerifyEmail = async (req, res) => {
    ...
    await collections.users.find({ username: { $in: req.body.users } }).forEach(async (doc) => {
        ...
        await NodemailerT.sendMail({
            ...
            to: doc.email,
            subject: "Email Verification for Cemara Training Platform", // Subject line
            ...
            if (!tooRecent) {
                ...
                try {
                    ...
                    const response = await NodemailerT.sendMail({
                        from: '"' + NodeCacheObj.get("emailSender") + '"' + ' <' + NodeCacheObj.get("emailSenderAddr") + '>', // sender address
                        to: user.email,
                        subject: "Password Reset Request for Cemara Training Platform", // Subject line
                        text: "Dear " + user.username + ",\n\n  Someone has requested a password reset for your Cemara Training Platform account. Your password reset link is: \n" + link + "\n\n The password link will expire in " + timeLeft + ". If you did not request this email, you can safely ignore this email.", // plain text body
                        html: `
                        Dear ${user.username},<br><br>  Someone has requested a password reset for your Cemara Training Platform account. To reset your password, click <a href='${link}'>here</a>. 
                        <br><br>
